...
## Decision 22: Native Terraform Policies vs EPAC

**Date**: 2025-12-20

**Context**: Considered using Enterprise Policy as Code (EPAC) for governance management.

| Option | Description | Trade-offs |
|--------|-------------|------------|
| **EPAC** | PowerShell-based framework for policies | ❌ Second language (PS)<br>❌ Separate state management<br>❌ High complexity |
| **Native Terraform (chosen)** | Standard `azurerm_policy_*` resources | ✅ Single language (HCL)<br>✅ Unified state/pipeline<br>✅ Consistent with Decision 6 |

**Decision**: Use native Terraform resources for Policy management to maintain architectural simplicity.

---

## Decision 23: Persistent Foundation-owned Observability

**Date**: 2025-12-20

**Context**: Logs generated by workloads would be lost if the workload resource group is destroyed.

| Option | Description | Trade-offs |
|--------|-------------|------------|
| **Per-env LAW** | Workspace inside the Workload RG | ❌ Logs destroyed with environment<br>❌ No forensic history |
| **Central LAW (chosen)** | One Workspace in Foundation (Root sub) | ✅ Persistent forensics<br>✅ Cross-env querying<br>✅ Cost efficient |

**Decision**: Deploy a single, persistent Log Analytics Workspace in the Foundation root subscription. All workloads ship logs here.

---

## Decision 24: Network Vending Model (Platform Scope)

**Date**: 2025-12-20

**Context**: Defining the boundary between the Platform team and Application teams.

**Decision**: The Platform repo provides "Network-as-a-Service". This includes VNets, Subnets, NSGs, and Route Tables. Application teams consume these pre-secured resources for their Compute (AKS/VMs).

---

## Decision 25: Shift-Left Security with Trivy

**Date**: 2025-12-20

**Context**: Need to catch infrastructure misconfigurations (e.g., open NSG rules) before they are deployed.

**Decision**: Integrated **Trivy** vulnerability scanning into GitHub Actions. Builds fail if "CRITICAL" or "HIGH" severity misconfigurations are detected in Terraform code.

---

## Decision 26: Decoupled CI/CD Approval Logic
